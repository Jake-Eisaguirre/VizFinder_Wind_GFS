...
# run times: 03:15PST, 09:15PST, 15:15PST, 21:15PST
```{r}
...
mod.list <- c("087", "090", "093", "096", "099", "102", "105", "108", "111", "114", "117", "120")
...
for(i in seq_along(mod.list)) {
  url_ucomp <- paste0("https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25_1hr.pl?file=gfs.t", mod_time, "z.pgrb2.0p25.f", mod.list[[i]] , "&lev_10_m_above_ground=on&var_UGRD=on&subregion=&leftlon=238.596&rightlon=242.936&toplat=35.00&bottomlat=32.36&dir=%2Fgfs.", date, "%2F", mod_time, "%2Fatmos")
...
for(i in seq_along(mod.list)) {
  ...
  url_ucomp <- paste0("https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25_1hr.pl?file=gfs.t", mod_time, "z.pgrb2.0p25.f", mod.list[[i]] , "&lev_10_m_above_ground=on&var_VGRD=on&subregion=&leftlon=238.596&rightlon=242.936&toplat=35.00&bottomlat=32.36&dir=%2Fgfs.", date, "%2F", mod_time, "%2Fatmos")
...
agg_u_vecs <- raster::aggregate(wind_u_brick, 1, mean) %>% 
...
agg_v_vecs <- raster::aggregate(wind_v_brick, 1, mean) %>% 
  mask(merged_shapes, inverse = T) %>% 
  rasterToPoints() %>% 
  as.data.frame() %>% 
  pivot_longer(!c(x,y), names_to = "date", values_to = "v") %>% 
